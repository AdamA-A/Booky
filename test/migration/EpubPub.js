class EpubPub {
    ...
    static async downloadEpub(epubFileName = "ruin-and-rising-by-leigh-bardugo.epub", contentUrl = "https://asset.epub.pub/epub/ruin-and-rising-by-leigh-bardugo-1.epub/content.opf") {
        ...
        var oebpsUrlInsert = realInfo.substring(epubFileName.length + 1, realInfo.substring(0, realInfo.lastIndexOf(".opf")).lastIndexOf('/') + 1); // EpubPub sometimes renames or foregoes an "OEBPS" folder, so this retrieves the name of the actual folder where OEBPS files are stored. EpubPub also sometimes calls the content.opf file something else, such as package.opf, so this accounts for those possible changes.
